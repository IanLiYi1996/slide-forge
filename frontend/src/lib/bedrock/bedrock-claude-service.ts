...
- 适合演示场合`;
  ...
  private buildContentGenerationSystemPrompt(language: string, tone: string): string {
    ...
    return `你是一位专业的演示文稿设计师。你的任务是根据用户提供的主题，生成一个完整的演示文稿大纲，包含详细的视觉设计指导。

## 输出要求

1. **语言**：使用${language}输出
2. **风格**：${toneDescriptions[tone] || toneDescriptions.professional}
3. **格式**：每张幻灯片必须包含以下四个部分
...
// NARRATIVE GOAL (叙事目标)
[1-2句话描述这张幻灯片的情感/策略目的 - 想要传达什么感觉或信息]
...
// LAYOUT (布局结构)
[1-2句话描述空间布局：
- 布局类型（海报式、网格、三联、居中、分屏等）
- 文字位置和图像元素的摆放
- 文字与图像的平衡]

## Slide 2: [幻灯片标题]
... (继续其他幻灯片，每张都包含完整的四个部分)

## 设计指南

1. 叙事目标：明确每张幻灯片想要传达的情感和信息
2. 关键内容：简洁有力，每个要点不超过 15 个字
3. 视觉画面：具体描述，便于 AI 图像生成器理解
4. 布局结构：专业的演示文稿设计排版
