...
import { SlideImageGenerator } from "../editor/SlideImageGenerator";
import { extractSlideContentAsText } from "@/lib/presentation/slide-content-extractor";
...
export function SlideEditPopover({ index }: SlideEditPopoverProps) {
  ...
  const [showSlideImageGenerator, setShowSlideImageGenerator] = useState(false);
  const updateSlide = (
    updates: Partial<{
  ...
  // Extract slide content as text for image generation
  const getSlideContentText = () => {
    if (!currentSlide) return "";
    return extractSlideContentAsText(currentSlide, index);
  };
    ...
    <>
      ...
      {currentSlide && (
        <SlideImageGenerator
          ...
          slideContent={getSlideContentText()}
          open={showSlideImageGenerator}
          onOpenChange={setShowSlideImageGenerator}
          onImageGenerated={(imageUrl) => {
            console.log("Slide image generated:", imageUrl);
            // Optionally update slide with the generated image URL
          }}
        />
      )}
        ...
        <div className="space-y-3">
          ...
          <div className="pb-2 border-b border-border">
            <Button
              ...
              onClick={() => {
                setShowSlideImageGenerator(true);
              }}
            >
              <Sparkles className="h-4 w-4" />
              Generate Slide Visualization
            </Button>
            <p className="text-xs text-muted-foreground mt-1.5 px-1">
              Create a complete visual representation of this slide
            </p>
          </div>
