...
import { Presentation, Clock, Trash2 } from "lucide-react";
import { useRouter } from "next/navigation";
import { formatDistanceToNow } from "date-fns";
import { useState } from "react";

export function RecentPreziSidebar() {
  ...
  const { data, isLoading } = useInfiniteQuery({
    queryKey: ["presentations-prezi-recent"],
  ...
  // Filter only Prezi presentations
  const allPresentations = data?.pages?.[0]?.items || [];
  const preziPresentations = allPresentations
    .filter((p) => p.presentation?.presentationMode === "PREZI")
    .slice(0, 5);

  // Handle delete
  const handleDeleteClick = (e: React.MouseEvent, presentationId: string) => {
    e.stopPropagation();
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast({
        title: "Deleted",
        description: "Prezi presentation deleted successfully",
      });

      // Refresh the list
      await queryClient.invalidateQueries({ queryKey: ["presentations-prezi-recent"] });
  ...
  if (preziPresentations.length === 0) {
    return null;
  }
  ...
  return (
    <div className="px-4 py-3">
      <div className="space-y-2">
        ...
        {preziPresentations.map((presentation) => (
          <div
            key={presentation.id}
            className="relative group"
          >
            <button
              onClick={() => router.push(`/presentation/prezi-edit/${presentation.id}`)}
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Prezi Presentation?</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to delete this Prezi presentation? This action cannot be undone.
              All elements, paths, and keyframes will be permanently deleted.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
